...
function resetImportedProject(){ files.clear(); assets.clear(); currentPath=''; selectedEntry=0; projectDirty=false; }
async function importFiles(list,folderMode=false){
  ...
  for(let i=0;i<arr.length;i++){
    const file=arr[i]; const rel=stripRoot(rawPaths[i],root); if(!rel||rel.endsWith('/'))continue;
    if(isLikelyText(rel)){
...
function finalizeImport(loaded,sourceLabel='resource'){
  ...
  projectDirty=false; runDiagnostics(); renderAll();
...
async function importZip(file){
  if(!file)return; try{
    ...
    for(const ent of entries){const path=stripRoot(ent.name,root);if(!path)continue;if(isLikelyText(path)){makeFile(path,dec.decode(ent.data),false,null);loaded++;}else assets.set(path,{bytes:ent.data});}
...
function runDiagnostics(){
  ...
  ids.light.forEach(x=>{const n=Number(x.id);if(!Number.isInteger(n)||n<0||n>255)addDiag('error','LIGHT_RANGE','lightSettings ID outside supported byte range',`Light ID ${x.id} is outside 0–255.`,x.path);});
  ids.siren.forEach(x=>{const n=Number(x.id),max=ssla?65534:254;if(!Number.isInteger(n)||n<1||n>max)addDiag('error','SIREN_RANGE','sirenSettings ID outside selected mode',`Siren ID ${x.id} is outside ${ssla?'SSLA extended range 1–65534':'vanilla-safe range 1–254'}.`,x.path);if(ssla&&[255,65535].includes(n))addDiag('error','SIREN_RESERVED','Reserved sirenSettings ID',`Siren ID ${x.id} is reserved and should not be used with SSLA.`,x.path);});
...
function generateManifest(){
  const author=$('#authorInput')?.value||'CowBoyKeno', version=$('#versionInput')?.value||'3.1.0', description=$('#descriptionInput')?.value||'Vehicle resource built with CBK Vehicle Meta Studio';
...
function mergedManifestText(existing){
  ...
  // Correct only exact-path registrations. Wildcard registrations are preserved because one pattern may intentionally cover many files.
  const expectedMap=new Map(expected.map(e=>[normalizePath(e.path).toLowerCase(),e]));
  text=text.replace(/\bdata_file\s*(?:\(\s*)?['"]([^'"]+)['"]\s*\)?\s*(?:\(\s*)?['"]([^'"]+)['"]/gi,(full,type,path)=>{const e=expectedMap.get(normalizePath(path).toLowerCase()),def=e?inferDef(e.path):null;return e&&def&&!manifestTypeAccepted(def,type)?full.replace(type,e.type):full;});
...
function manifestPreviewText(){const mp=findManifestPath();if(mp&&basename(mp).toLowerCase()==='fxmanifest.lua')return mergedManifestText(files.get(mp).text);return generateManifest();}
function refreshManifest(create=true){
  const existingPath=[...files.keys()].find(p=>basename(p).toLowerCase()==='fxmanifest.lua'); const text=existingPath?mergedManifestText(files.get(existingPath).text):generateManifest();
...
function downloadText(text,name,mime='text/plain'){downloadBlob(new Blob([text],{type:mime}),name)}
function exportCurrent(){const f=currentFile();if(!f)return;downloadText(f.text,basename(currentPath),isXmlFile(currentPath)?'application/xml':'text/plain');f.dirty=false;renderAll();}
...
async function buildZip(){
  runDiagnostics(); const errors=diagnostics.filter(d=>d.level==='error'); if(errors.length&&!confirm(`Diagnostics found ${errors.length} error${errors.length===1?'':'s'}. Build the ZIP anyway?`))return;
  const enc=new TextEncoder(), entries=[], forceGenerated=!!$('#preferGeneratedManifest')?.checked; const fx=[...files.keys()].find(p=>basename(p).toLowerCase()==='fxmanifest.lua'), legacy=[...files.keys()].find(p=>basename(p).toLowerCase()==='__resource.lua');
  for(const [path,f] of files){const b=basename(path).toLowerCase();if(forceGenerated&&(b==='fxmanifest.lua'||b==='__resource.lua'))continue;if(!forceGenerated&&fx&&b==='fxmanifest.lua')continue;entries.push({name:path,bytes:enc.encode(f.text)});}
  if(forceGenerated)entries.unshift({name:'fxmanifest.lua',bytes:enc.encode(generateManifest())}); else if(fx)entries.unshift({name:fx,bytes:enc.encode(files.get(fx).text)}); else if(!legacy)entries.unshift({name:'fxmanifest.lua',bytes:enc.encode(generateManifest())});
  if($('#includeAssets').checked){for(const [path,entry] of assets)entries.push({name:path,bytes:await assetBytes(entry)});}else if(assets.size&&!confirm(`This resource has ${assets.size} imported binary/stream file${assets.size===1?'':'s'} and they are excluded from the ZIP. Build metadata only?`))return;
  const blob=await makeZip(entries), name=($('#projectName').value.trim()||'cbk_vehicle').replace(/[^a-zA-Z0-9_-]/g,'_')+'.zip'; downloadBlob(blob,name); projectDirty=false;files.forEach(f=>f.dirty=false);renderAll();toast(`Built ${name} with ${entries.length} file${entries.length===1?'':'s'}.`);
}
...
function switchTab(name){currentTab=name;$$('.tab').forEach(b=>b.classList.toggle('active',b.dataset.tab===name));$$('.tab-panel').forEach(p=>p.classList.add('hidden'));$('#'+name+'Tab').classList.remove('hidden');if(name==='raw')renderRaw();if(name==='diagnostics'){runDiagnostics();renderDiagnostics()}if(name==='builder'){$('#manifestPreview').value=manifestPreviewText();}if(name==='reference')renderReference();}
function renderAll(){renderHeader();renderFileList();renderProjectState();if(currentTab==='smart')renderSmart();else if(currentTab==='raw')renderRaw();else if(currentTab==='diagnostics')renderDiagnostics();else if(currentTab==='builder')$('#manifestPreview').value=manifestPreviewText();else if(currentTab==='reference')renderReference();}
...
$('#exportCurrentBtn').onclick=exportCurrent; $('#exportZipBtn').onclick=buildZip; $('#builderZipBtn').onclick=buildZip;
